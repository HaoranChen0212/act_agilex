...
import wandb
from utils_xyyaw import load_data, compute_dict_mean, set_seed, detach_dict
...
def train(args):
    ...
    train_dataloader, val_dataloader, stats, _ = load_data(dataset_dir, num_episodes, args.arm_delay_time,
                                                           ...
                                                           args.batch_size, args.batch_size, relative_action_config,
                                                           args.use_all_dirs_for_val)
...
def forward_pass(policy_config, data, policy):
    # DEBUG _
    image_data, image_depth_data, qpos_data, action_data, action_is_pad, _= data
...
def train_process(train_dataloader, val_dataloader, config, stats):
    wandb.init(
        ...
        project=config.get("wandb_project", "act_mobile_manip"),
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        wandb.log(
            {f"val/{k}": v.item() for k, v in epoch_summary.items()} ,
            step=global_step
        )
        # training
        policy.train()
        ...
        for batch_idx, data in enumerate(train_dataloader):
            ...
            wandb.log(
                {
                    "lr/main": optimizer.param_groups[0]["lr"],
        ...
        print(summary_string)
        wandb.log(
            {f"train/{k}": v.item() for k, v in epoch_summary.items()},
            step=global_step
        )
...
def get_arguments():
    ...
    parser.add_argument('--dataset_dir', action='store', nargs="+", type=str, help='dataset_dir', required=True)
    parser.add_argument('--exp_name', action='store', type=str, help='', required=False)
    ...
    parser.add_argument('--camera_names', action='store', nargs="+", type=str,
                        help='camera names used for training image inputs',
                        default=['cam_high', 'cam_left_wrist', 'cam_right_wrist'], required=False)
    parser.add_argument('--use_all_dirs_for_val', action='store', type=parse_bool_arg,
                        help='if true, split train/val across all dataset_dir inputs',
...
if __name__ == '__main__':
    main()
# python act/train.py --dataset_dir /home/agilex/data --task_name go_get_banana_copy --use_robot_base True --pretrain_ckpt policy_best.ckpt --ckpt_dir ~/train_dir/ --num_episodes 10 --batch_size 10 --num_epochs 2000 
